...
// 러버니(토끼 동상) 오브젝트 스크립트 (즉사형)
// [매뉴얼 3번] 우리의 귀여운 토끼 친구 '러버니'는 항상 정면을 응시하고 있습니다.
//             만약 그렇지 않다면, 손전등을 끄고 지나가세요. 손전등을 켠 채로 지나가면 러버니와 눈이 마주칠 수 있습니다.
// 이상 상태일 때 판정 범위 안에서 손전등을 켜면 눈이 마주쳐 게임오버된다.
public class RubaniAnomaly : Anomaly
{
    [Header("Rubani Visuals")]
    [SerializeField] private SpriteRenderer spriteRenderer; // 상태를 표시할 렌더러
    [Tooltip("정상 상태(정면 응시) 스프라이트")]
    [SerializeField] private Sprite normalSprite;  // 정상(정면 응시) 스프라이트
    [Tooltip("이상 상태(정면을 보지 않음) 스프라이트")]
    [SerializeField] private Sprite anomalySprite; // 이상(정면 안 봄) 스프라이트
    ...
    private void Update()
    {
        ...
        if (distance <= detectRange && playerFlashlight.IsOn)
        {
            ...
            {
                GameManager.Instance.GameOver("러버니와 눈이 마주쳤습니다.");
            }
            else
            {
                Debug.LogWarning("[GameOver] 러버니와 눈이 마주쳤습니다. (GameManager 없음)");
            }
        }
    }
}